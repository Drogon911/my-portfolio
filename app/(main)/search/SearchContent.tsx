...
export default function SearchContent({
  ...
  allTracks: SearchTrack[];
}) {
  const { currentTrack } = usePlayer();
  const searchParams = useSearchParams();
  const router = useRouter();

  // fuse и карта плейлистов строятся один раз на каталог (useMemo), а не на уровне
  // модуля — данные теперь приходят пропсом. Стабильные ссылки нужны, чтобы
  // React.memo у PlayButton не ломался на каждом рендере результатов.
  const fuse = useMemo(
    () =>
      new Fuse(allTracks, {
        keys: [
