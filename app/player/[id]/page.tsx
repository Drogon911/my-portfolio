  ...
  return (
    <div className="min-h-screen flex flex-col relative overflow-hidden">
      <div className="absolute top-4 left-4 z-50">
        <Link href="/" className="inline-flex items-center gap-2 px-4 py-2 bg-white/20 backdrop-blur-md rounded-full text-white text-sm font-medium hover:bg-purple-500/40 transition-all duration-300 border border-white/30 shadow-lg">
          ← На главную
        </Link>
      </div>
      ...
      <div className="relative z-10 flex-1 flex flex-col">
        <div className="flex-1 flex flex-col items-center justify-center p-6">
          <AnimatePresence mode="wait">
            <motion.div key={currentTrackIndex} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.2 }} className="relative w-64 h-64 md:w-80 md:h-80 rounded-2xl shadow-2xl overflow-hidden bg-white/10 backdrop-blur-sm">
              <Image src={currentTrack.cover} alt={currentTrack.title} fill sizes="(max-width: 768px) 100vw, 33vw" className="object-cover" priority />
            </motion.div>
          </AnimatePresence>

          <AnimatePresence mode="wait">
            <motion.h1 key={currentTrack.title} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.15 }} className="text-3xl md:text-4xl font-bold text-white mt-8 text-center drop-shadow-lg">
              {currentTrack.title}
            </motion.h1>
          </AnimatePresence>
          <p className="text-pink-100 mt-2 text-center drop-shadow">{album.name} — {album.artist}</p>
        </div>

        <div className="w-full bg-white/20 backdrop-blur-lg border-t border-white/30 shadow-2xl rounded-t-3xl p-6 pb-8">
          <div className="max-w-2xl mx-auto w-full">
            <input type="range" min="0" max={duration || 0} value={progress} onChange={handleSeek} className="w-full h-1.5 rounded-lg appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-pink-600 [&::-webkit-slider-thumb]:shadow-md [&::-webkit-slider-thumb]:cursor-pointer" style={{ background: `linear-gradient(to right, #ec4899 0%, #ec4899 ${(progress / (duration || 1)) * 100}%, #fbcfe8 ${(progress / (duration || 1)) * 100}%, #fbcfe8 100%)` }} />
          ...
          <div className="flex gap-8 justify-center items-center mt-6">
            ...
            <button onClick={togglePlay} className="p-5 rounded-full bg-white hover:bg-pink-50 transition shadow-lg text-pink-600 text-3xl">{isPlaying ? "⏸" : "▶"}</button>
            <button onClick={nextTrack} className="p-3 rounded-full bg-white/30 hover:bg-white/50 transition text-white text-2xl">⏭</button>
          </div>

          <audio ref={audioRef} src={currentTrack.src} onPlay={() => setIsPlaying(true)} onPause={() => setIsPlaying(false)} onEnded={nextTrack} />
        </div>
      </div>
